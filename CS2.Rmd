...
registers <- registers %>%
  select(-c(election_date, stats_type, update_date)) %>%
...
data <- registers %>%
  ...
  left_join(votes, by = c(
    "county_desc", "age", "party_cd", "race_code", "ethnic_code", "sex_code"
  ))
```
...
data <- data %>%
  mutate(
    total_vot = as.numeric(total_vot), 
...
data %>%
  ...
  ggplot( aes(x = county_desc, y = turnout_rate)) +
    ...
    ggtitle("Turnout rate vs. County") +
...
In order to answer the questions of interest, we require the following features in our model:

1. Fixed effects for various demographic subgroups (race, ethnicity, age, sex)
2. Random effects for county
3. Sex-Party interaction
4. Age-Party interaction
...
```{r}
as.data.frame(summary(mod1)$coefficients) %>%
  mutate(`95% CI` = paste0(
    '[', 
...
# mod2 <- glmer(resp ~ 1 + party_cd + (1 | county_desc),
#               data = train, family = binomial)

as.data.frame(summary(mod2)$coefficients) %>%
  mutate(`95% CI` = paste0(
    '[', 
    round(Estimate - 1.96 * `Std. Error`, 3), ', ', 
    round(Estimate + 1.96 * `Std. Error`, 3), ']' )
...
predicted <- predict(mod2, test, type="response")
