...
```{r, warning= F, fig.height= 4, fig.width= 10, echo = FALSE}
p1 <- data %>%
...
p2 <- data %>%
  ...
  geom_text(aes(label=format(n, digits = 2)), hjust = -0.1) +
...
```{r, warning= F, fig.height= 3, fig.width= 12, echo = FALSE}
p13 <- data %>%
...
p14 <- data %>%
  ...
  geom_bar(stat = "identity") +
  xlab("Sex * Party") + ylab("Size") +
  geom_text(aes(label=format(n, digits = 2)), hjust = -0.05) +
  coord_flip() +
  ggtitle("Registered Voters vs. Sex*Party") +
    theme(plot.title = element_text(hjust = 0.5))
...
```{r, warning= F, fig.height= 3.5, fig.width= 12, echo = FALSE}
p15 <- data %>%
...
```{r, warning= F, fig.height= 3.8, fig.width= 12, echo = FALSE}
p17 <- data %>%
...
&=\beta_0 + \sum_{k_p} \beta_{1k_p} \mathbb{I}[P_{ij}=k_p] + \sum_{k_r} \beta_{2k_r} \mathbb{I}[R_{ij}=k_r] + \sum_{k_e} \beta_{3k_e} \mathbb{I}[E_{ij}=k_e] + \sum_{k_s} \beta_{4k_s} \mathbb{I}[S_{ij}=k_s]
\\
&+ \sum_{k_a} \beta_{5k_a} \mathbb{I}[A_{ij}=k_a] + \sum_{k_s,k_p} \beta_{6k_sk_p} \mathbb{I}[S_{ij}=k_s] \mathbb{I}[P_{ij}=k_p] + \sum_{k_a,k_p} \beta_{7k_ak_p} \mathbb{I}[A_{ij}=k_a] \mathbb{I}[P_{ij}=k_p]
\\
&+ \sum_{k_a,k_r} \beta_{8k_ak_r} \mathbb{I}[A_{ij}=k_a] \mathbb{I}[R_{ij}=k_r] + b_{0j}
\end{aligned}
$$
...
* $P_{ij}$ is the Party code ($k_p \in \{DEM,\ LIB,\ REP,\ UNA \}$)

* $R_{ij}$ is the Race code ($k_r \in \{Undesignated,\ B,\ W,\ A,\ I,\ O\}$)

* $E_{ij}$ is the Ethnic code ($k_e \in \{NL,\ UN\}$)

* $S_{ij}$ is the sex code ($k_s \in \{M,\ U\}$)

* $A_{ij}$ is the Age group ($k_a \in \{Age \ 20-40,\ Age \ 41-65,\ Age \ Over \ 66\}$)
...
chart_fixef <- function(model) {
 summary(model)$coefficients %>%
    as.data.frame() %>%
    mutate(`95% lower` = round(Estimate - 1.96 * `Std. Error`, 4),
           `95% upper` = round(Estimate + 1.96 * `Std. Error`, 4)) %>%
...
a CST/GRE voter with undesignated gender has $e^{0.1790} = 1.1960$ times (a 20% increase) the odds of voting
...
chart_fixef(mod6)[19:26,] %>%
  kable(caption = "Estiamtes and 95% CI of Fixed Effects for Party:Sex",
        digits=4, format = "markdown") %>%
  kable_styling(full_width=FALSE,latex_options = c("HOLD_position", "striped")) %>%
    kable_classic()
```

From the table above, only the fixed effects for `party_cdLIB:sex_codeU` is significant (the 95% CI does not cover 0). For simplicity, we only interpret this fixed effect: A Libertarian voter with undesignated gender has $e^{0.4754} = 1.6087$ times the odds of voting as a female of the same party, holding all other predictors unchanged.
...
chart_fixef(mod6)[27:38,] %>%
  kable(caption = "Estiamtes and 95% CI of Fixed Effects for Party:Age",
        digits=4, format = "markdown") %>%
  kable_styling(full_width=FALSE,latex_options = c("HOLD_position", "striped")) %>%
    kable_classic()
```
...
From the table above, we see some of the fixed effects are significant, we only interpret them:

A 26-40 year old Democratic voter has $e^{-0.2391} = 0.7873$ times the odds of voting

A 41-65 year old Democratic voter has $e^{0.4162} = 1.5162$ times the odds of voting

A 41-65 year old Republican voter has $e^{0.5018} = 1.6517$ times the odds of voting

A 41-65 year old voter with undesignated party has $e^{0.5563} = 1.7442$ times the odds of voting

An over 66 year old Republican voter has $e^{0.5102} = 1.6656$ times the odds of voting

An over 66 year old voter with undesignated party has $e^{0.8719} = 2.3915$ times the odds of voting

as a 18-25 year old of the same party, holding all other predictors unchanged.

## Strengths and Limitations

**Strengths:** A major strength of our model is the inclusion of random intercepts by county which allows us to understand the difference in turnout rate between each county. 

**Limitations:** Firstly, we dropped rows that contained missing values during data cleaning. While we are working with almost 2 million observations, we do have quite a few small demographic groups, especially for our interaction terms. A possible alternative would have been to impute values for the missing data so that we could keep as many rows as possible. Obviously imputation comes with it's own set of assumptions and limitations. In addition, we did not try poststratiﬁcation to re-adjust the proportions among demographic groups. This could lead to slightly biased result.
...
```{r, warning= F, fig.width= 12, fig.height= 4, echo = FALSE}
p3 <- data %>%
  group_by(race_code) %>%
  summarise(total_reg = sum(total_reg),
            total_vot = sum(total_vot), .groups = "drop") %>%
...
```{r, warning= F, fig.width= 10, fig.height= 4, echo = FALSE}
p5 <- data %>%
  group_by(party_cd) %>%
  summarise(total_reg = sum(total_reg),
            total_vot = sum(total_vot), .groups = "drop") %>%
...
```{r, warning= F, fig.width= 10, fig.height= 3, echo = FALSE}
p7 <- data %>%
  group_by(ethnic_code) %>%
  summarise(total_reg = sum(total_reg),
            total_vot = sum(total_vot), .groups = "drop") %>%
...
```{r, echo = FALSE}
chart_fixef(mod6) %>%
  kable(caption = "Estiamtes and 95% CI of Fixed Effects", longtable = T,
        digits=4, format = "markdown") %>%
...
```{r, echo = FALSE}
ranef(mod6, condVar = TRUE) %>%
  as.data.frame() %>%
  arrange(desc(condval)) %>%
  mutate(`exp(condval)` = exp(condval)) %>%
  kable(caption = "Estimated random intercepts",longtable = T, 
        digits =4, format = "markdown") %>%
