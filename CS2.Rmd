...
data %>%
  group_by(county_desc) %>%
  summarize(n = n())
# no extremely small sample size
```
...
data %>%
  group_by(race_code) %>%
  summarize(n = n())
# race_code p has a small sample size, drop it
...
```{r}
mod4 <- glmer(cbind(cbind(total_vot, total_reg - total_vot)) ~
                  party_cd + race_code + ethnic_code + sex_code + age +
...
LRT <- c("",
         ...
         round(anova(mod1,mod3)$`Pr(>Chisq)`[2],4),
         round(anova(mod3,mod4)$`Pr(>Chisq)`[2],4))
BIC_score1 <- sapply(c(mod1, mod2, mod3, mod4), BIC)
